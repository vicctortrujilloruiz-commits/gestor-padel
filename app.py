...
def limpiar_para_pdf(texto):
    """Elimina emojis y caracteres que no encajan en Latin-1 para FPDF."""
    if texto is None:
        return ""
    texto_str = str(texto)
    ...
    for orig, rep in reemplazos.items():
        texto_str = texto_str.replace(orig, rep)
    # Conservar solo caracteres Latin-1 imprimibles
    caracteres_validos = []
    for ch in texto_str:
        try:
            ch.encode("latin-1")
            caracteres_validos.append(ch)
        except UnicodeEncodeError:
            caracteres_validos.append(" ")
    return re.sub(r"\s+", " ", "".join(caracteres_validos)).strip()
class PDFTorneo(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, limpiar_para_pdf("CUADRO OFICIAL Y HORARIOS DEL TORNEO"), align="C", new_x="LMARGIN", new_y="NEXT")
        self.set_font("Helvetica", "I", 10)
        self.set_text_color(100, 116, 139)
        fecha_txt = datetime.now().strftime('%d/%m/%Y %H:%M')
        self.cell(0, 6, limpiar_para_pdf(f"Generado con GeneradorPadel - {fecha_txt}"), align="C", new_x="LMARGIN", new_y="NEXT")
        self.ln(5)
    def footer(self):
        self.set_y(-15)
...
def exportar_cuadro_pdf():
    ...
    tipo_txt = limpiar_para_pdf(formato_info.get("tipo", "Torneo").upper())
    ...
    pistas_str = limpiar_para_pdf(", ".join(st.session_state.pistas))
    pdf.cell(90, 6, f"Pistas: {len(st.session_state.pistas)} ({pistas_str})")
    ...
    if rondas:
        ...
        pdf.cell(0, 8, limpiar_para_pdf("PARTIDOS DE LA FASE PREVIA / LIGUILLA"), new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
        for num_r, r in enumerate(rondas, start=1):
            pdf.set_font("Helvetica", "B", 10)
            pdf.set_fill_color(226, 232, 240)
            pdf.cell(0, 7, limpiar_para_pdf(f" Ronda {num_r}"), fill=True, new_x="LMARGIN", new_y="NEXT")
            ...
            for p in r.get("partidos", []):
                ...
                pa = limpiar_para_pdf(f"{p['pareja_a'][0]}/{p['pareja_a'][1]}")
                pb = limpiar_para_pdf(f"{p['pareja_b'][0]}/{p['pareja_b'][1]}")
                dia_txt = limpiar_para_pdf(p.get("dia", "-"))
                hora_txt = limpiar_para_pdf(p.get("hora", "-"))
                pista_txt = limpiar_para_pdf(p.get("pista", "-"))
                pdf.cell(20, 6, dia_txt[:10], border=1)
                pdf.cell(16, 6, hora_txt[:8], border=1)
                pdf.cell(24, 6, pista_txt[:12], border=1)
    ...
    if partidos_elim:
        ...
        ronda_tit = limpiar_para_pdf(nombre_ronda(len(st.session_state.get("cuadro_actual", []))))
        pdf.cell(0, 8, limpiar_para_pdf(f"CUADRO ELIMINATORIO ({ronda_tit})"), new_x="LMARGIN", new_y="NEXT")
        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 9)
        pdf.set_fill_color(248, 250, 252)
        pdf.cell(20, 6, "Dia", border=1, fill=True)
...
def _firma_estado_pdf():
    partes = [
        st.session_state.get("etapa"),
        st.session_state.get("ronda_eliminatoria_num"),
        len(st.session_state.get("rondas_programadas", [])),
        len(st.session_state.get("partidos_ronda_actual", [])),
        st.session_state.get("campeon"),
        bool(st.session_state.get("logo_torneo_bytes")),
    ]
    resultados = []
    for r in st.session_state.get("rondas_programadas", []):
        for p in r.get("partidos", []):
            resultados.append((p.get("sets_a"), p.get("sets_b")))
    for p in st.session_state.get("partidos_ronda_actual", []):
        resultados.append((p.get("sets_a"), p.get("sets_b")))
    partes.append(tuple(resultados))
    return hash(str(partes))
def obtener_pdf_cacheado():
    firma_actual = _firma_estado_pdf()
    if (st.session_state.get("_pdf_cache_firma") != firma_actual
            or st.session_state.get("_pdf_cache_bytes") is None):
        st.session_state["_pdf_cache_bytes"] = exportar_cuadro_pdf()
        st.session_state["_pdf_cache_firma"] = firma_actual
    return st.session_state["_pdf_cache_bytes"]
# ==================================================================
...
CAMPOS_TORNEO = [
    ...
    "partidos_ronda_actual", "partidos_ronda_actual_num",
    "partidos_ronda_actual_sin_hueco", "campeon",
...
def cargar_torneo(contenido_json):
    ...
    st.session_state["_pdf_cache_firma"] = None
...
def inicializar_estado():
    defaults = {
        ...
        "partidos_ronda_actual_num": -1, "partidos_ronda_actual_sin_hueco": False,
        "campeon": None,
        "liguilla_partido": None, "liguilla_campeon": None,
        "slot_index_acumulado": 0, "logo_torneo_bytes": None,
        "_pdf_cache_firma": None, "_pdf_cache_bytes": None
    }
...
def panel_configuracion():
    ...
    with st.sidebar.expander("🕐 Restricciones horarias", expanded=False):
        st.caption("ℹ️ Indica únicamente las horas en las que una pareja **NO puede jugar**.")
        restricciones = {}
        opciones_dias_restriccion = ["Todos"] + [d["etiqueta"] for d in dias_ui]
        
        parejas_con_nombre = [(idx, p) for idx, p in enumerate(nombres) if (p[0] or p[1])]
        
        if not parejas_con_nombre:
            st.info("Introduce los nombres de las parejas arriba para asignar restricciones.")
        
        for idx, pareja in parejas_con_nombre:
            nombre_mostrar = f"{pareja[0] or f'J1'} / {pareja[1] or f'J2'}"
            
            # Tarjeta independiente para cada pareja
            with st.container(border=True):
                tiene_restr = st.checkbox(
                    f"⛔ **{nombre_mostrar}**",
                    key=f"restr_check_{idx}",
                    help=f"Activa si {nombre_mostrar} tiene incompatibilidad horaria."
                )
                
                if tiene_restr:
                    num_restr = st.number_input(
                        "Bloqueos para esta pareja:",
                        min_value=1,
                        max_value=5,
                        value=1,
                        step=1,
                    ...
                    for r in range(1, num_restr + 1):
                        st.markdown(f"🔒 *Incompatibilidad #{r}*")
                        dia_r = st.selectbox(
                            "Día afectado:",
                        ...
                        with rc1:
                            desde = st.time_input(
                                "No puede desde:",
                                value=datetime(2026, 1, 1, 9, 0).time(),
                        ...
                        with rc2:
                            hasta = st.time_input(
                                "No puede hasta:",
                                value=datetime(2026, 1, 1, 14, 0).time(),
                        ...
                        if desde >= hasta:
                            ...
                            st.caption(f"🚫 **Bloqueo:** {dia_r} de {desde.strftime('%H:%M')} a {hasta.strftime('%H:%M')}")
    ...
    # ------------------------------------------------------------
    # Comprobación de capacidad horaria: si no caben todos los
    ...
    if not permitido:
        ...
        generar = st.sidebar.button(
            ...
            key="btn_unico_generar",
            disabled=capacidad_insuficiente
        )
        if capacidad_insuficiente:
            st.sidebar.caption("⛔ Botón deshabilitado hasta que haya huecos horarios suficientes para todos los partidos.")
    if generar:
        # Guarda de seguridad adicional: nunca crear el torneo si falta capacidad,
        # aunque el botón se hubiera podido pulsar en un estado intermedio.
        if capacidad_insuficiente:
            st.sidebar.error("🚫 No se puede crear el torneo: faltan huecos horarios para todos los partidos.")
            return
        ...
        if len(nombres_validos) != num_parejas:
            ...
            return
        nombres_normalizados = [(n1.strip().lower(), n2.strip().lower()) for n1, n2 in nombres_validos]
        if len(set(nombres_normalizados)) != len(nombres_normalizados):
            st.sidebar.error("⚠️ Hay parejas con nombres duplicados. Cada pareja debe tener un nombre distinto.")
            return
...
def _asignar_ronda_eliminatoria(equipos, pistas, dias, duracion, restricciones, parejas, start_slot_index):
    idx_por_pareja = {pareja: idx for idx, pareja in enumerate(parejas)}
    numero_equipos = len(equipos)
    pendientes = [
        (equipos[i], equipos[numero_equipos - 1 - i])
        for i in range(numero_equipos // 2)
    ]
    slots = obtener_slots(dias, duracion)
    slot_index = start_slot_index
    partidos_ronda = []
    while pendientes and slots and slot_index < len(slots):
        slot = slots[slot_index]
        hora_inicio = slot["hora"]
        hora_fin = hora_inicio + timedelta(minutes=duracion)
        for pista in pistas:
            if not pendientes:
                break
            elegido = None
            for emparejamiento in pendientes:
                equipo_a, equipo_b = emparejamiento
                idx_a = idx_por_pareja.get(equipo_a)
                idx_b = idx_por_pareja.get(equipo_b)
                disponible = (
                    (idx_a is None or pareja_disponible(idx_a, slot["dia"], hora_inicio, hora_fin, restricciones))
                    and (idx_b is None or pareja_disponible(idx_b, slot["dia"], hora_inicio, hora_fin, restricciones))
                )
                if disponible:
                    elegido = emparejamiento
                    break
            if elegido is None:
                continue
            pendientes.remove(elegido)
            equipo_a, equipo_b = elegido
            partidos_ronda.append({
                "pareja_a": equipo_a, "pareja_b": equipo_b,
                "pista": pista, "dia": slot["dia"], "hora": hora_inicio.strftime("%H:%M"),
                "sets_a": None, "sets_b": None, "juegos_a": None, "juegos_b": None
            })
        slot_index += 1
    hay_sin_hueco = bool(pendientes)
    if pendientes:
        dia_fallback = dias[-1]["etiqueta"] if dias else "Día Final"
        for i, (equipo_a, equipo_b) in enumerate(pendientes):
            partidos_ronda.append({
                "pareja_a": equipo_a, "pareja_b": equipo_b,
                "pista": pistas[i % len(pistas)] if pistas else "Pista 1",
                "dia": dia_fallback, "hora": "Por asignar",
                "sets_a": None, "sets_b": None, "juegos_a": None, "juegos_b": None
            })
    return partidos_ronda, slot_index, hay_sin_hueco
def panel_eliminatoria():
    ...
    if (st.session_state.partidos_ronda_actual_num != ronda_num or 
        ...
        partidos_ronda, nuevo_slot_index, hay_sin_hueco = _asignar_ronda_eliminatoria(
            equipos, pistas, dias, duracion, st.session_state.restricciones,
            st.session_state.parejas, st.session_state.slot_index_acumulado
        )
        st.session_state.slot_index_acumulado = nuevo_slot_index
        st.session_state.partidos_ronda_actual = partidos_ronda
        st.session_state.partidos_ronda_actual_num = ronda_num
        st.session_state.partidos_ronda_actual_sin_hueco = hay_sin_hueco
    partidos_ronda = st.session_state.partidos_ronda_actual
    if st.session_state.get("partidos_ronda_actual_sin_hueco"):
        st.warning("⚠️ Algún partido de esta ronda no encontró hueco libre respetando pistas, horarios y restricciones, y se ha colocado con hora 'Por asignar'.")
    ...
    if todos_jugados:
        ...
        if len(ganadores) == 1:
            ...
            if st.button(f"➡️ Avanzar a {nombre_ronda(len(ganadores))}", type="primary"):
                ...
                st.session_state.partidos_ronda_actual = []
                st.session_state.partidos_ronda_actual_num = -1
                st.session_state.partidos_ronda_actual_sin_hueco = False
                st.rerun()
def panel_final():
