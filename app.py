...
def _firma_estado_pdf():
    """Genera una firma ligera del estado relevante para el PDF, para no
    tener que regenerarlo en cada rerun de Streamlit si nada ha cambiado."""
    partes = [
...
def obtener_pdf_cacheado():
    """Devuelve el PDF del torneo, regenerándolo solo si el contenido
    relevante ha cambiado desde la última vez (resultados, ronda, logo...).
    Evita reconstruir el PDF completo en cada clic de la interfaz."""
...
def cargar_torneo(contenido_json):
    ...
    # Al cargar un torneo, invalidamos la caché del PDF para que se regenere.
...
def panel_configuracion():
    ...
    # --- Aviso de capacidad (pistas x franjas horarias vs partidos necesarios) ---
    ...
    if generar:
        ...
        if len(set(nombres_normalizados)) != len(nombres_normalizados):
            st.sidebar.error("⚠️ Hay parejas con nombres duplicados. Cada pareja debe tener un nombre distinto (usa un apodo o apellido si hace falta).")
            return
        ...
        # Invalidamos la caché del PDF: hay un torneo nuevo.
        st.session_state["_pdf_cache_firma"] = None
        st.session_state["_pdf_cache_bytes"] = None
        st.rerun()
# ==================================================================
...
def _asignar_ronda_eliminatoria(equipos, pistas, dias, duracion, restricciones, parejas, start_slot_index):
    """Programa los emparejamientos de una ronda eliminatoria respetando
    las restricciones horarias de cada pareja y repartiendo los partidos
    entre todas las pistas disponibles dentro del mismo slot (igual que en
    la fase previa), en vez de ponerlos uno detrás de otro en pistas fijas.
    Devuelve (partidos_ronda, nuevo_indice_de_slot, hubo_partidos_sin_hueco).
    """
...
def panel_eliminatoria():
    ...
    # Reconstrucción y asignación con slots (Día, hora, pista), respetando
    # restricciones horarias y repartiendo entre pistas disponibles.
    ...
    if st.session_state.get("partidos_ronda_actual_sin_hueco"):
        st.warning("⚠️ Algún partido de esta ronda no encontró hueco libre respetando pistas, horarios y restricciones, y se ha colocado con hora 'Por asignar'. Revísalo manualmente antes de jugarlo.")
...
def main():
    ...
    with st.sidebar:
        ...
        # Descarga de PDF (se cachea y solo se regenera si cambian los datos)
        if st.session_state.etapa != "config":
