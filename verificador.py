...
def _verificar_licencia_pro(codigo: str, email_usuario: str) -> bool:
    ...
    - Si YA existe: se concede acceso solo si el email coincide (sin
...
def es_pago_valido(codigo_pago: str, email_usuario: str = "") -> bool:
    ...
    - 11,99 €: pi_/ch_ válido; la propiedad del código la decide
      la tabla licencias_pro (no el email que Stripe tenga guardado).
    """
...
def marcar_como_usado(codigo_pago: str):
    """
    Quema el código de PASE INDIVIDUAL (2,99€) justo después de generar
    el torneo, para que no pueda reutilizarse. Los códigos de Licencia
    PRO no se tocan aquí: ya quedaron vinculados a su email en
    `licencias_pro` desde es_pago_valido().
    """
    try:
        supabase = get_supabase_client()
